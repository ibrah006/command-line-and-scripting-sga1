...
The version of linux kernel currently in use by the system is displayed
...
The output ensures there is network connectivity. If there is not network connection, command will output an error
...
The output displays the current time in the first column, it has been up for 1 hour and 40 mins, 1 user logged into the system and displays the load on each processing core, the higher value indicates the high load in the system
