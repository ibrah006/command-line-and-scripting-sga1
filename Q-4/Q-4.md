# Question 4

1.
```bash
uptime
``` 
![alt text](image.png)
The system has been up for 1 hour and 30 mins

2. 
```bash
ps -u $USER
```
![alt text](image-1.png)
All the processes running on he current user account

3. 
```bash
top
```
![alt text](image-2.png)
we can see that process with IDs 2202 and 4902 are consuming highest CPU usage with 2.3%

4.
we can run the sleep command
```bash
sleep 100 & 
``` 
this command sleeps (times out) for 25 seconds and then runs the command that comes after &
![alt text](image-3.png)
we can verify this job is running while it is by runnning:
```bash
jobs
```
output:
```
[1]+ Running        sleep 25 &
```

when it finishes running the `jobs` will ouput he following:
```
[1]+ Done        sleep 25 &
```

5.
```bash
pu -u $USER -o pid,pcpu,comm
``` 
![alt text](image-4.png)
the order of columns for the output is changes to Process ID, %CPU and Command name

6. 
```bash
free -h
```
![alt text](image-5.png)
total memory: 3.3Gib
<br>
used: 1.2Gib
<br>
free: 523Mib
<br>
shared: 44Mib
<br>
buff/cache: 1.74ib
<br>
available: 2.1Gib

no swap memory used

7. 
```bash
df -h ~
```
![alt text](../Q-3//image-12.png)
shows the disk usage for the filesystem containing the specified directory - in this case, the home directory

8. 
```bash
echo $SHELL
```
![alt text](image-6.png)
Bash is the shell in use by the user

9. 
```bash
uname -a > system_report.txt
```
![alt text](image-7.png)

10. 
```bash
ncdu ~
```
![alt text](image-8.png)
scans the home directory

ncdu is a an interactive disk usage analyzer which lets us see folder/file sizes sorted by largest first. It is easier and faster than du for exploring disk space and other things related to disk and space
